...
export function RentDialog({ offer, open, onOpenChange }: Props) {
  ...
  function handleSubmit() {
    ...
    rentClore.mutate(req, { onSuccess: () => onOpenChange(false), onError: (e) => setError(e.message) });
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="default">
        <DialogHeader>
          <DialogTitle>Rent {offer.gpu_name}</DialogTitle>
          <DialogDescription>
        ...
        <DialogBody>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Docker image</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">SSH authentication</label>
          ...
          {orderType === "spot" && (
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Max spot price ($/day)</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Port mappings (JSON)</label>
          ...
          {showAdvanced && (
            <div className="space-y-3">
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Environment variables (JSON)</label>
                <Input className="text-sm font-mono" value={envRaw} onChange={(e) => setEnvRaw(e.target.value)} placeholder='{"HF_TOKEN": "hf_..."}' />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Startup command</label>
                <Input className="text-sm" value={command} onChange={(e) => setCommand(e.target.value)} placeholder="bash -c 'pip install vllm && ...'" />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Jupyter token</label>
